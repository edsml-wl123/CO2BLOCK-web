...
function co2density = calculate_co2_density(T, p)
    % This function calculates the density of CO2 based on temperature and pressure.
    % 
    % Inputs:
    ...
    % 
    % Output:
    % co2density - Density of CO2 in kg/m^3

    % Define constants
    a0 = 7.54;  % Constant [Pa·m^6·K^0.5·mol^-2]
    a1 = -4.13*10^-3;  % Constant [Pa·m^6·K^0.5·mol^-2]
    b = 2.78*10^-5;  % Constant [m^3/mol]
    R = 8.314472;  % Gas constant [m^3·Pa·K^-1·mol^-1]
    ...
    initial_guess = 1e-4;  % Appropriate initial guess value, adjust as needed

    % Use fsolve for numerical solving
    options = optimset('Display', 'off');  % Turn off display
    V = fsolve(eqn, initial_guess, options);  % Solve the equation
